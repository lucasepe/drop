...
# Costruzione per darwin (macOS) architetture amd64 e arm64
for ARCH in ${ARCHS[@]}; do
  if [[ "$ARCH" == "amd64" ]]; then
    OUTPUT_DIR="${DIST_DIR}/macos-x86_64"
    ARCHIVE_FILE=${DIST_DIR}/${PROJECT_NAME}-macos-x86_64
  else
    OUTPUT_DIR="${DIST_DIR}/macos-${ARCH}"
    ARCHIVE_FILE=${DIST_DIR}/${PROJECT_NAME}-macos-${ARCH}
  fi
  ...
  #upx --best --lzma --brute "$OUTPUT_FILE"
  ...
  echo "Built and archived ${ARCHIVE_FILE}.zip for macOS ($ARCH)"
